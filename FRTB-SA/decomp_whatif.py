...
sql="select * from TB_26_FRTB_SENSITIVITY_TRADE_DETAIL_WHATIF;"
cursor.execute(sql)
...
Raw_Data = Raw_Data[(Raw_Data['DATA_DATE']==p_data_date)]
...
pos=pd.concat([GIRR_delta,GIRR_vega,GIRR_curvature,CSR_delta,CMTY_delta,CMTY_vega,CMTY_curvature,FX_delta,FX_vega,FX_curvature],join="outer",ignore_index=True)

pos.loc[:,'SEC_TRANCHE']=np.nan
...
bucket=pd.concat([GIRR_delta_agg,GIRR_vega_agg,GIRR_curvature_agg,CSR_delta_agg,CMTY_delta_agg,CMTY_vega_agg,CMTY_curvature_agg,FX_delta_agg,FX_vega_agg,FX_curvature_agg],join="outer",ignore_index=True)
...
class_=pd.concat([girrd, girrv, girrc, csrd,cmtyd, cmtyv, cmtyc, fxd, fxv, fxc],ignore_index=True)
...
'''
########## result: risk factor##########
cursor.execute("delete from `TB_26_FRTB_SENSITIVITY_RESULT_RISKFACTOR_WHATIF` where DATA_DATE = %s",p_data_date)
db.commit()
...
for i,row in pos.iterrows():
    sql1 = "INSERT INTO `TB_26_FRTB_SENSITIVITY_RESULT_RISKFACTOR_WHATIF` (`" +cols1 + "`) VALUES (" + "%s,"*(len(row)-1) + "%s)"
    cursor.execute(sql1, tuple(row))
db.commit()
'''

########## result: bucket ##########
cursor.execute("delete from `TB_26_FRTB_SENSITIVITY_RESULT_BUCKET_WHATIF` where DATA_DATE = %s",p_data_date)
db.commit()
...
for i,row in bucket.iterrows():
    sql2 = "INSERT INTO `TB_26_FRTB_SENSITIVITY_RESULT_BUCKET_WHATIF` (`" +cols2 + "`) VALUES (" + "%s,"*(len(row)-1) + "%s)"
    cursor.execute(sql2, tuple(row))
db.commit()

########## result: class ##########
cursor.execute("delete from `TB_26_FRTB_SENSITIVITY_RESULT_RISKCLASS_WHATIF` where DATA_DATE = %s",p_data_date)
db.commit()
...
for i,row in class_.iterrows():
    sql3 = "INSERT INTO `TB_26_FRTB_SENSITIVITY_RESULT_RISKCLASS_WHATIF` (`" +cols3 + "`) VALUES (" + "%s,"*(len(row)-1) + "%s)"
    cursor.execute(sql3, tuple(row))
db.commit()

########## decomp: risk factor##########
cursor.execute("delete from `TB_26_FRTB_SENSITIVITY_DECOMP_RISKFACTOR_WHATIF` where DATA_DATE = %s",p_data_date)
db.commit()
...
for i,row in riskfactor.iterrows():
    sql4 = "INSERT INTO `TB_26_FRTB_SENSITIVITY_DECOMP_RISKFACTOR_WHATIF` (`" +cols4 + "`) VALUES (" + "%s,"*(len(row)-1) + "%s)"
    cursor.execute(sql4, tuple(row))
db.commit()

########## decomp: portfolio ##########
cursor.execute("delete from `TB_26_FRTB_SENSITIVITY_DECOMP_PORTFOLIO_WHATIF` where DATA_DATE = %s",p_data_date)
db.commit()
...
for i,row in level3.iterrows():
    sql5 = "INSERT INTO `TB_26_FRTB_SENSITIVITY_DECOMP_PORTFOLIO_WHATIF` (`" +cols5 + "`) VALUES (" + "%s,"*(len(row)-1) + "%s)"
